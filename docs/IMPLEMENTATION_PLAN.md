...
- `compose.send`: send or queue compose windows through Thunderbird. MCP send tools require `confirmSend=true` and default to `sendLater`.
...
- `send_reply`: composes and sends or queues a reply, requiring `confirmSend=true`.
...
- `build/thunderbird-mcp-bridge-0.1.3.xpi`
- Native host manifest and wrapper generated by `npm run install-native`
- `build/thunderbird-mcp-0.1.3.mcpb`
